...
class Customer(models.Model):
    ...
    customer_note   客户备注
    status      状态
    network_consult_note    网络咨询师咨询内容
    date        咨询日期
    last_consult_date       最后跟进日期
    next_date   预计再次跟进时间
    network_consultant      FK咨询师
    consultant  FK销售
    class_list  ManyToMany已报班级
    """
    ...
    def show_status(self):
        color_dict = {'signed': "green",
                      'unregistered': "red",
                      'studying': 'blue',
                      'paid_in_full': "yellow"}
        return '<span style="color: black;background-color: {};padding: 5px">{}</span>'.format(
...
class Enrollment(models.Model):
    ...
    why_us      报名原因
    your_expectation    期望
    contract_agreed     同意协议
    contract_approved   是否审批通过
    memo        备注
    delete_status       删除状态
    customer    FK客户
    school      FK校区
    enrolment_class     FK所报班级
    """
